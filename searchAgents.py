...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state: Any):
        ...
        """
        # state[1] -> visited corners
        return set(state[1]) == set(self.corners) # visite all corners
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                successors.append(((next_state, tuple(visited_corners)), action, 1))
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    if not unvisited_corners: # we have no corners to visit
        return 0

    while unvisited_corners:
        closest_corner = unvisited_corners[0]
        closest_corner_distance = util.manhattanDistance(current_position, closest_corner)

        for corner in unvisited_corners:
            current_distance = util.manhattanDistance(current_position, corner)
...
def foodHeuristic(state: Tuple[Tuple, List[List]], problem: FoodSearchProblem):
    ...
    if len(foods) == 0: # there's no food
        return 0
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        return search.bfs(problem) # we could use also ucs
